...
void AABB::transform(const Mat4& transform) {
  ...
  const Vec3 new_edge = Vec3(glm::abs(transform[0][0]) * old_edge.x + glm::abs(transform[1][0]) * old_edge.y + glm::abs(transform[2][0]) * old_edge.z,
...
bool Sphere::intersects(const AABB& b) const {
  const Vec3 closestPointInAabb = glm::min(glm::max(center, b.min), b.max);
...
bool Sphere::intersects(const Sphere& b, float& dist, Vec3& direction) const {
...
bool Sphere::intersects(const RayCast& b, float& dist, Vec3& direction) const {
  auto C = center;
  auto O = b.get_origin();
