...
namespace ox {
auto EasyRenderPipeline::init(vuk::Allocator& allocator) -> void {
  ...
  sky_transmittance_lut_view.create({},
                                    TextureLoadInfo{.preset = vuk::ImageAttachment::Preset::eSTT2DUnmipped,
                                                    .format = vuk::Format::eR16G16B16A16Sfloat,
                                                    .extent = {.width = 256u, .height = 64u, .depth = 1u}});
  ...
  auto [_, temp_atmos_buffer] = vuk::create_cpu_buffer(allocator, std::span(&temp_atmos_info, 1));

  auto transmittance_lut_attachment = sky_transmittance_lut_view.discard("sky_transmittance_lut");

  std::tie(transmittance_lut_attachment, temp_atmos_buffer) =
      vuk::make_pass("transmittance_lut_pass",
  ...
  std::tie(transmittance_lut_attachment, multiscatter_lut_attachment, temp_atmos_buffer) =
      vuk::make_pass("sky_multiscatter_lut_pass",
  ...
  transmittance_lut_attachment =
      transmittance_lut_attachment.as_released(vuk::eComputeSampled, vuk::DomainFlagBits::eGraphicsQueue);
  multiscatter_lut_attachment =
      multiscatter_lut_attachment.as_released(vuk::eComputeSampled, vuk::DomainFlagBits::eGraphicsQueue);

  vuk::Compiler compiler{};
  transmittance_lut_attachment.wait(allocator, compiler);
  multiscatter_lut_attachment.wait(allocator, compiler);

  exposure_buffer = vuk::allocate_cpu_buffer(allocator, sizeof(GPU::HistogramLuminance));
}

auto EasyRenderPipeline::shutdown() -> void {}

auto EasyRenderPipeline::on_render(vuk::Allocator& frame_allocator,
                                   const RenderInfo& render_info) -> vuk::Value<vuk::ImageAttachment> {
  OX_SCOPED_ZONE;
  ...
  std::vector<GPU::CameraData> camera_datas = {};
  camera_datas.emplace_back(camera_data);
  auto [_, camera_buffer] = vuk::create_cpu_buffer(frame_allocator, std::span(camera_datas));
  ...
  auto [_, material_buffer] = vuk::create_cpu_buffer(frame_allocator, std::span(gpu_materials));

  if (render_queue_2d.sprite_data.empty())
    render_queue_2d.sprite_data.emplace_back();
  auto [_, vertex_buffer_2d] = vuk::create_cpu_buffer(frame_allocator, std::span(render_queue_2d.sprite_data));
  ...
  if (this->atmosphere.has_value()) {
    sky_pass(frame_allocator, camera_buffer, final_attachment, depth_attachment);
  }

  auto histogram_inf = this->histogram_info.value_or(GPU::HistogramInfo{});

  auto histogram_buffer = vuk::allocate_gpu_buffer(frame_allocator, GPU::HISTOGRAM_BIN_COUNT * sizeof(u32));
  auto histogram_buffer_value = vuk::acquire_buf("histogram buffer", *histogram_buffer, vuk::Access::eNone);
  vuk::fill(histogram_buffer_value, 0);

  std::tie(final_attachment, histogram_buffer_value) = vuk::make_pass(
      "histogram generate",
  ...
  })(std::move(final_attachment), std::move(histogram_buffer_value));

  auto exposure_buffer_value = vuk::acquire_buf("exposure buffer", *exposure_buffer, vuk::eNone);

  if (histogram_info.has_value()) {
    exposure_buffer_value =
        vuk::make_pass("histogram average",
                       [pixel_count = f32(final_attachment->extent.width * final_attachment->extent.height),
                        histogram_inf](vuk::CommandBuffer& cmd_list,
      ...
      return exposure;
    })(std::move(histogram_buffer_value), std::move(exposure_buffer_value));
  }

  result_attachment = vuk::make_pass("tonemap",
                                     [](vuk::CommandBuffer& cmd_list,
...
auto EasyRenderPipeline::sky_pass(vuk::Allocator& frame_allocator,
  ...
  auto sky_aerial_perspective_attachment =
      vuk::declare_ia("sky aerial perspective",
  ...
  auto [_, atmosphere_buffer] = vuk::create_cpu_buffer(frame_allocator, std::span(&this->atmosphere, 1));

  auto [_, sun_buffer] = vuk::create_cpu_buffer(frame_allocator, std::span(&this->sun, 1));

  auto sky_transmittance_lut_attachment =
      sky_transmittance_lut_view.acquire("sky_transmittance_lut", vuk::Access::eComputeSampled);
  auto sky_multiscatter_lut_attachment =
      sky_multiscatter_lut_view.acquire("sky_multiscatter_lut", vuk::Access::eComputeSampled);
  ...
  std::tie(sky_aerial_perspective_attachment,
           ...
           sun_buffer,
           camera_buffer) =
      vuk::make_pass("sky aerial perspective",
                     [&descriptor_set = *this->descriptor_set_00](vuk::CommandBuffer& cmd_list,
                                                                  VUK_BA(vuk::eComputeRead) atmosphere_,
                                                                  VUK_BA(vuk::eComputeRead) sun_,
                                                                  VUK_BA(vuk::eComputeRead) camera,
  ...
  std::tie(final_attachment, depth_attachment, camera_buffer) = vuk::make_pass(
    ...
    cmd_list.bind_graphics_pipeline("sky_final_pipeline")
        ...
        .set_rasterization({})
        .set_depth_stencil(
            {.depthTestEnable = false, .depthWriteEnable = false, .depthCompareOp = vuk::CompareOp::eGreaterOrEqual})
