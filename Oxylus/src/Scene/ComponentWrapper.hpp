...
struct ComponentWrapper {
  ...
  inline ComponentWrapper(flecs::entity& holder_,
    ...
    members_data = static_cast<uint8*>(holder_.get_mut(comp_id_));
  }
  ...
  inline void for_each(this ComponentWrapper& self,
    ...
    for (usize i = 0; i < self.member_count; i++) {
      ...
      if (member_type == flecs::F32) {
        data = reinterpret_cast<float32*>(self.members_data + member.offset);
      } else if (member_type == flecs::I32) {
        data = reinterpret_cast<int32*>(self.members_data + member.offset);
      } else if (member_type == flecs::U32) {
        data = reinterpret_cast<uint32*>(self.members_data + member.offset);
      } else if (member_type == flecs::I64) {
        data = reinterpret_cast<int64*>(self.members_data + member.offset);
      } else if (member_type == flecs::I64) {
        data = reinterpret_cast<uint64*>(self.members_data + member.offset);
