...
namespace ox {
void SceneRenderer::init(Scene* scene, const Shared<RenderPipeline>& render_pipeline) {
  OX_SCOPED_ZONE;

  _scene = scene;

  if (!render_pipeline)
    _render_pipeline = create_unique<EasyRenderPipeline>();
  else
    _render_pipeline = render_pipeline;

  _render_pipeline->init(App::get_vkcontext());
}
    ...
    _scene->world.query_builder<SpriteComponent, SpriteAnimationComponent>().build().each(
        [&delta_time](SpriteComponent& sprite, SpriteAnimationComponent& sprite_animation) {
          ...
          if (sprite_animation.num_frames < 1 || sprite_animation.fps < 1 || sprite_animation.columns < 1 ||
              !material || !material->albedo_texture)
            return;
