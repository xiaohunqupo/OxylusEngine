...
template<>
struct option_flag_val<std::uint8_t> {
    constexpr static auto nullopt = std::numeric_limits<std::uint8_t>::max();
};

template<>
struct option_flag_val<std::uint16_t> {
    constexpr static auto nullopt = std::numeric_limits<uint16_t>::max();
};

template<>
struct option_flag_val<std::uint32_t> {
    constexpr static auto nullopt = std::numeric_limits<std::uint32_t>::max();
};

template<>
struct option_flag_val<std::size_t> {
    constexpr static auto nullopt = std::numeric_limits<std::size_t>::max();
};

template<>
struct option_flag_val<float, std::enable_if_t<std::numeric_limits<float>::is_iec559>> {
    constexpr static auto nullopt = static_cast<float>(0x7fedb6db);
};

template<>
struct option_flag_val<double, std::enable_if_t<std::numeric_limits<double>::is_iec559>> {
    constexpr static auto nullopt = static_cast<double>(0x7ffdb6db6db6db6d);
};
