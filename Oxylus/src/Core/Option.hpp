...
private:
  static_assert(!std::is_same_v<std::nullopt_t,
                                std::remove_cvref_t<decltype(option_flag_val<T>::nullopt)>>,
...
public:
  ...
  template <typename U = T,
            std::enable_if_t<std::is_constructible_v<T,
                                                     U&&>,
                             int> = 0>
  constexpr option_flag(U&& v) noexcept(std::is_nothrow_assignable_v<T&,
                                                                     U> &&
  ...
  template <typename U,
            typename SelfT>
  [[nodiscard]] constexpr T value_or(this SelfT&& self,
                                     U&& default_value) {
  ...
  template <typename SelfT,
            typename FuncT>
  constexpr auto&& or_else(this SelfT&& self,
                           FuncT&& fun) {
    if (self) {
      return self;
    }
  ...
  template <typename U = T,
            std::enable_if_t<std::is_constructible_v<T,
                                                     U&&>,
                             int> = 0>
  option_flag& operator=(U&& _new) noexcept(std::is_nothrow_assignable_v<T&,
                                                                         U> &&
                                            std::is_nothrow_constructible_v<T,
                                                                            U>) {
    if (has_value()) {
  ...
  template <typename U,
            std::enable_if_t<std::conjunction_v<std::is_constructible<T,
                                                                      const U&>,
                                                std::is_assignable<T&,
                                                                   const U&>>,
                             int> = 0>
  ...
  template <typename U,
            std::enable_if_t<std::conjunction_v<std::is_constructible<T,
                                                ...
                                                std::is_assignable<T&,
                                                                   U>>,
                             int> = 0>
  option_flag& operator=(option_flag<U>&& other) noexcept(std::is_nothrow_assignable_v<T&,
                                                                                       T> &&
                                                          std::is_nothrow_constructible_v<T,
                                                                                          T>) {
    if (other.has_value()) {
...
template <typename T,
          typename U>
bool operator==(const option_flag<T>& opt,
                const U& value) {
