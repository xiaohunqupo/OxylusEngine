...
void bind_debug_renderer(const Shared<sol::state>& state) {
  ...
  debug_table.set_function("draw_point", [](const glm::vec3& point, glm::vec3 color) -> void { DebugRenderer::draw_point(point, 1.0f, glm::vec4(color, 1.0f)); });
  debug_table.set_function("draw_line", [](const glm::vec3& start, const glm::vec3& end, const glm::vec3& color = glm::vec3(1)) -> void {
