...
void bind_math(const Shared<sol::state>& state) {
  ...
  auto uvec2 = state->new_usertype<glm::uvec2>("UVec2", sol::constructors<glm::uvec2(uint32_t, uint32_t), glm::uvec2(uint32_t)>());
  ...
  auto vec3 = state->new_usertype<glm::vec3>("Vec3", sol::constructors<sol::types<>, sol::types<float, float, float>, glm::vec3(float)>());
  ...
  auto ivec3 = state->new_usertype<glm::ivec3>("IVec3", sol::constructors<sol::types<>, sol::types<int, int, int>, glm::ivec3(int)>());
  ...
  auto uvec3 = state->new_usertype<glm::uvec3>("UVec3", sol::constructors<sol::types<>, sol::types<uint32_t, uint32_t, uint32_t>, glm::uvec3(uint32_t)>());
  ...
  auto vec4 = state->new_usertype<glm::vec4>("Vec4", sol::constructors<sol::types<>, sol::types<float, float, float, float>, glm::vec4(float)>());
  ...
  auto ivec4 = state->new_usertype<glm::ivec4>("IVec4", sol::constructors<sol::types<>, sol::types<int, int, int, int>, glm::ivec4(int)>());
  ...
  auto uvec4 = state->new_usertype<glm::uvec4>("UVec4",
  ...
  state->new_usertype<glm::mat3>("Mat3",
                            sol::constructors<glm::mat3(float, float, float, float, float, float, float, float, float), glm::mat3(float), glm::mat3()>(),
                            sol::meta_function::multiplication,
                            [](const glm::mat3& a, const glm::mat3& b) { return a * b; });

  state->new_usertype<glm::mat4>("Mat4", sol::constructors<glm::mat4(float), glm::mat4()>(), sol::meta_function::multiplication, [](const glm::mat4& a, const glm::mat4& b) {
    return a * b;
  ...
  glm_table.set_function("translate", [](const glm::mat4& mat, const glm::vec3& vec) { return glm::translate(mat, vec); });
  ...
  glm_table.set_function("length", sol::overload([](const glm::vec2& vec) { return glm::length(vec); }, [](const glm::vec3& vec) {
    return glm::length(vec);
  }, [](const glm::vec4& vec) { return glm::length(vec); }));
  glm_table.set_function("normalize", sol::overload([](const glm::vec2& vec) { return glm::normalize(vec); }, [](const glm::vec3& vec) {
