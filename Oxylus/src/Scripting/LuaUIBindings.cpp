...
void bind_ui(const Shared<sol::state>& state) {
  ...
  uitbl.set_function("property_bool_tooltip",
                     [](const char* label, bool* flag, const char* tooltip) { ui::property(label, flag, tooltip); });
  uitbl.set_function("property_input_field", [](const char* label, std::string* text, ImGuiInputFlags flags) {
    ui::property(label, text, flags);
  });
  uitbl.set_function("property_input_field_tooltip",
                     [](const char* label, std::string* text, ImGuiInputFlags flags, const char* tooltip) {
    ui::property(label, text, flags, tooltip);
  });
